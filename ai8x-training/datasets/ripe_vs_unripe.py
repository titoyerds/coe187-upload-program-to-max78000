...
class RipevsUnripe(Dataset):

    labels = ['ripe', 'unripe']
    label_to_id_map = {k: v for v, k in enumerate(labels)}
    label_to_folder_map = {'ripe': 'riped', 'unripe': 'unriped'}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(64, 64), augment_data=False):
        self.root_dir = root_dir
        self.data_dir = os.path.join(root_dir, 'ripe_vs_unripe', d_type)

        if not self.__check_ripevsunripe_data_exist():
        ...
        if d_type == 'train' and augment_data:
            self.album_transform = album.Compose([
                # Resize FIRST to drastically reduce memory usage
    ...
    def __check_ripevsunripe_data_exist(self):
...
def get_ripevsunripe_dataset(data, load_train, load_test):
    """
    ...
    if load_train:
        train_dataset = RipevsUnripe(root_dir=data_dir, d_type='train',
                                   transform=transform, augment_data=True)
    else:
        train_dataset = None

    if load_test:
        test_dataset = RipevsUnripe(root_dir=data_dir, d_type='test', transform=transform)
    else:
        test_dataset = None
...
datasets = [
    {
        ...
        'input': (3, 64, 64),
        'output': ('ripe', 'unripe'),
        'loader': get_ripevsunripe_dataset,
    },
]